...
class Zelda1Env(NESEnv):
    ...
    def __init__(self):
        ...
        self.anchors_visited = set()
        self.last_screens = None

        self.last_action = None
        self.last_direction = None
        self.moveActions = {'$08', '$04', '$01', '$02'}
    ...
    def _get_reward(self):
        ...
        screen = self.ram[0xEB]
        #Player1 Buttons Pressed $08=Up, $04=Down, $01=Right, $02=Left $80=A, $40=B, $20=Select, $10=Start
        actionMade = self.ram[0xFA]

        # Link's Direction $08=North, $04=South, $01=East, $02=West
        linkDirection = self.ram[0x98]
        reward = 0
        ...
        if(screen != self.last_screens):
            if(not(screen in self.screens_visited)):
                reward += 100
                self.screens_visited.add(screen)

            self.anchor_pixel_x = int(self._x_pixel)
            self.anchor_pixel_y = int(self._y_pixel)
            self.old_dx = 0
            self.old_dy = 0

            self.last_screens = screen

        else:
            dx = np.absolute(int(self._x_pixel) - self.anchor_pixel_x)
            dy = np.absolute(int(self._y_pixel) - self.anchor_pixel_y)
            anchorCoordinate = (dx, dy)
            reward += (dx - self.old_dx) * 2
            reward += (dy - self.old_dy) * 2

            # negative reward to visit same coordinate
            if anchorCoordinate not in self.anchors_visited:
                self.anchors_visited.add(anchorCoordinate)
            else:
                reward -= 5

            #Positive reward according to continuous action and direction
            if (dx - self.old_dx) > 0 or (dy - self.old_dy) > 0:
                #if current action is only movement and direction + action are same as last action
                if self.last_action == actionMade \
                    and self.last_action == linkDirection \
                    and actionMade in self.moveActions:
                    reward += 5
                #if current action is only movement and direction
                elif self.last_action == actionMade \
                    and actionMade in self.moveActions:
                    reward += 3

                elif self.last_direction == linkDirection \
                    and actionMade in self.moveActions:
                    reward += 3
            ...
            self.old_dx = dx
            self.old_dy = dy
            self.last_action = actionMade
            self.last_direction = linkDirection
